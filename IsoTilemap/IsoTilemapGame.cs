...
namespace IsoTilemap
{
    ...
    /// </summary>
    public class IsoTilemapGame : Game
    {
        ...
        protected override void LoadContent()
        {
            ...
            var tex2D = m_SpriteManager.CreateTexture2D("spritesheets/debug-tileset");

            m_Tileset = new RegularTileset("test-tileset", tex2D, 10, 10, 64, 64);

            var tilesets = new List<ITileset>();
            tilesets.Add(m_Tileset);

            MapLayer layer = new MapLayer("ground layer", 10, 10);

            for(int y = 0; y < 10; y++)
            {
                for(int x = 0;x < 10; x++)
                {
                    int idx = m_RNG.Next(0,100);
            ...
            m_TileMap = new TileMap(m_SpriteManager, TileMapType.Isometric, 64, 32, tilesets, layers);
        }
